...
function buildSemesterInputs() {
  ...
  document.querySelectorAll('.semester-skip').forEach(box => box.addEventListener('change', () => {document.querySelectorAll(`.subject-score[data-semester="${box.dataset.semester}"]`).forEach(input => input.disabled = box.checked);markCalculationPending()}));
  document.querySelectorAll('.subject-score').forEach(input=>input.addEventListener('change',markCalculationPending));
}
function markCalculationPending(){if(!$('calculationStatus'))return;$('totalScore').textContent='총점 -';$('calculationStatus').textContent='성적이 변경되었습니다. 지원 가능 학교를 다시 확인하세요.'}
function activateTab(id) { const current=document.querySelector('.tab.is-active')?.dataset.tab, protectedTabs=['manage','stats']; if(protectedTabs.includes(current) && !protectedTabs.includes(id)) managementUnlocked=false; document.querySelectorAll('.tab').forEach(x => x.classList.toggle('is-active', x.dataset.tab === id)); document.querySelectorAll('.tab-panel').forEach(x => x.classList.toggle('is-active', x.id === id)); }
function teacherPassword(){try{return String(JSON.parse(localStorage.getItem('odong-config-v2')||'{}').teacherPin||'6673')}catch(error){return'6673'}}
function saveTeacherPassword(pin){let config={};try{config=JSON.parse(localStorage.getItem('odong-config-v2')||'{}')}catch(error){}config.teacherPin=pin;localStorage.setItem('odong-config-v2',JSON.stringify(config))}
...
function bindEvents() {
  $('calculateButton').onclick = () => { $('studentName').value=$('studentName').value.trim();if(!$('studentName').value){$('studentName').focus();return alert('학생 이름을 입력하세요.')}const common=commonInputError();if(common)return alert(common);const eligible=eligibleSchoolScores();if(!eligible.length){$('totalScore').textContent='총점 -';$('calculationStatus').textContent='계산 가능한 3-1 또는 전체학기 입시요강이 없습니다.';return alert('입력한 성적으로 계산할 수 있는 학교가 없습니다. 3-1 또는 전체학기 입시요강과 필수 과목을 확인하세요.')}saveStudentInfo();recordPrediction();renderResults();renderSearch();renderStats(); };
  ['grade','classNo','studentNo'].forEach(key=>$(key).onchange=saveStudentInfo);$('studentName').onchange=()=>{$('studentName').value=$('studentName').value.trim();saveStudentInfo()};$('attendance').oninput=markCalculationPending;$('volunteer').oninput=markCalculationPending;$('clearStudentInfo').onclick=clearStudentInfo;
  ...
  $('addSenior').onclick = addSeniorAdmission; $('downloadTemplate').onclick = downloadAdmissionTemplate; $('importExcel').onclick = importAdmissionFile; $('seniorSchool').onchange = () => $('seniorCustomSchoolBox').hidden = $('seniorSchool').value !== '__direct__'; $('saveThresholds').onclick = updateThresholds;
}
...
function activeCriteria(){try{return JSON.parse(localStorage.getItem('odong-school-settings-v2')||'[]').filter(c=>['적용','적용 가능'].includes(c.status))}catch(error){return[]}}
function criteriaSemesterLabel(criteria){const enabled=['2-1','2-2','3-1'].filter(x=>criteria?.semesters?.[x]?.enabled!==false);if(enabled.length===1&&enabled[0]==='3-1')return'3-1 기준';if(enabled.length===3)return'전체학기 기준';return''}
function activeCriteriaForSchool(name){return activeCriteria().filter(c=>c.schoolName===name&&criteriaSemesterLabel(c))}
function evaluateStudentFormula(source,vars){const names=Object.keys(vars),tokens=[];let p=0,i=0,s=String(source||'').replaceAll('×','*').replaceAll('÷','/').replace(/\s/g,'');if(!s||/[;{}\[\]'".`]/.test(s))throw new Error('산출식 오류');while(i<s.length){if(/[+\-*/()]/.test(s[i])){tokens.push(s[i++]);continue}const m=s.slice(i).match(/^\d+(?:\.\d+)?/);if(m){tokens.push(Number(m[0]));i+=m[0].length;continue}const name=names.find(x=>s.startsWith(x,i));if(name){tokens.push(name);i+=name.length;continue}throw new Error('산출식 변수 오류')}const primary=()=>{const x=tokens[p++];if(x==='('){const v=expr();if(tokens[p++]!==')')throw new Error('괄호 오류');return v}if(x==='-')return-primary();if(typeof x==='number')return x;if(names.includes(x))return Number(vars[x]||0);throw new Error('산출식 오류')},term=()=>{let v=primary();while(tokens[p]==='*'||tokens[p]==='/'){const op=tokens[p++],r=primary();if(op==='/'&&!r)throw new Error('0 나누기 오류');v=op==='*'?v*r:v/r}return v},expr=()=>{let v=term();while(tokens[p]==='+'||tokens[p]==='-'){const op=tokens[p++],r=term();v=op==='+'?v+r:v-r}return v};const result=expr();if(p!==tokens.length||!Number.isFinite(result))throw new Error('산출식 오류');return result}
function studentScore(school=null) {
  const selected=school||schools.find(s=>s.name===$('preferredSchool').value),candidate=bestSchoolCandidate(selected);
  return candidate?.result.ok?candidate.result.total:null;
}
function commonInputError(){const absenceRaw=$('attendance').value.trim(),hoursRaw=$('volunteer').value.trim(),absence=Number(absenceRaw),hours=Number(hoursRaw);if(absenceRaw===''||!Number.isFinite(absence)||absence<0)return'미인정 결석일수를 0 이상의 숫자로 입력하세요.';if(hoursRaw===''||!Number.isFinite(hours)||hours<0)return'봉사시간을 0 이상의 숫자로 입력하세요.';return''}
function schoolScoreResult(school,criteria){
  if(!criteria)return{ok:false,reason:'3-1 또는 전체학기 입시요강 미등록'};const common=commonInputError();if(common)return{ok:false,reason:common};
  const inputs=[...document.querySelectorAll('.subject-score')],absence=Number($('attendance').value),hours=Number($('volunteer').value);let sum=0,count=0,weighted=0,weightSum=0;const missing=[];
  inputs.forEach((input,index)=>{const semesterIndex=Number(input.dataset.semester),semesterKey=['2-1','2-2','3-1'][semesterIndex],subject=SUBJECTS[index%SUBJECTS.length];if(criteria.semesters?.[semesterKey]?.enabled===false||criteria.subjects?.[subject]?.enabled===false)return;const skipped=document.querySelector(`.semester-skip[data-semester="${semesterIndex}"]`).checked,grade=Number(input.value);if(skipped||!input.value||input.value==='x'||!Number.isFinite(grade)){missing.push(`${semesterKey} ${subject}`);return}const weight=Number(criteria.semesters?.[semesterKey]?.weight||1)*Number(criteria.subjects?.[subject]?.weight||1);sum+=grade;count++;weighted+=grade*weight;weightSum+=weight});
  if(missing.length||!count||weightSum<=0)return{ok:false,reason:'필수 반영학기 성적 미입력',missing};const vars={평점합:sum,평점평균:sum/count,반영과목수:count,최고평점합:5*count,가중평점합:weighted,가중치합:weightSum,최고가중평점합:5*weightSum,교과만점:Number(criteria.academicMax||0),전체교과가중치:Number(criteria.academicWeight||1)},academicScore=criteria.useAcademicFormula!==false?evaluateStudentFormula(criteria.academicFormula,vars):weighted/(5*weightSum)*Number(criteria.academicMax||0),rules=criteria.nonAcademic||{};
  ...
  const total=academicScore+non;return{ok:Number.isFinite(total),reason:Number.isFinite(total)?'':'산출식 계산 오류',total,academicScore,nonAcademicScore:non,criteria,semesterLabel:criteriaSemesterLabel(criteria)};
}
function schoolCandidates(school){return activeCriteriaForSchool(school?.name).map(criteria=>({school,result:schoolScoreResult(school,criteria),criteria}))}
function eligibleSchoolScores(){return schools.flatMap(s=>schoolCandidates(s)).filter(x=>x.result.ok)}
function schoolCutoff(s,criteria=null) { if(criteria)return Number(criteria.expected);const scores = seniorAdmissions.filter(x => x.school === s.name).map(x => Number(x.score)).filter(Number.isFinite); return scores.length ? Math.min(...scores) : Number(s.cutoff); }
...
function judgementForResult(s,result,criteria) { const score=result?.ok?result.total:null,cutoff=schoolCutoff(s,criteria);if(score===null)return[result?.reason||'성적 입력 필요','challenge'];if(!Number.isFinite(cutoff)||cutoff<=0)return['자료 없음','challenge'];const gap=score-cutoff;return gap>=thresholds.stable?['안정','safe']:gap>=thresholds.suitable?['적정','match']:gap>=thresholds.challenge?['소신','challenge']:['어려움','challenge']}
function bestSchoolCandidate(s){const valid=schoolCandidates(s).filter(x=>x.result.ok);return valid.sort((a,b)=>(b.result.total-schoolCutoff(s,b.criteria))-(a.result.total-schoolCutoff(s,a.criteria)))[0]||null}
function judgement(s) { const candidate=bestSchoolCandidate(s);return candidate?judgementForResult(s,candidate.result,candidate.criteria):['성적 입력 필요','challenge'] }
...
function renderResults() {
  const filter = $('resultRegion').value, eligible=eligibleSchoolScores(),list=eligible.filter(({school:s}) => filter === '전체' || (filter === '경주·포항' ? s.region === filter : s.region !== '경주·포항'));
  const preferred=schools.find(s=>s.name===$('preferredSchool').value),preferredCandidates=preferred?schoolCandidates(preferred).filter(x=>x.result.ok):[];
  $('totalScore').textContent=preferredCandidates.length?`총점 ${preferredCandidates.map(x=>`${x.result.semesterLabel.replace(' 기준','')} ${x.result.total.toFixed(1)}점`).join(' / ')}`:'총점 -';
  $('calculationStatus').textContent=eligible.length?`${new Set(eligible.map(x=>x.school.name)).size}개 학교 · ${eligible.length}개 판정기준이 계산되었습니다.`:'계산 가능한 3-1 또는 전체학기 입시요강이 없습니다.';
  $('resultRows').innerHTML = list.length?list.map(({school:s,result,criteria}) => { const [label,style]=judgementForResult(s,result,criteria),cutoff=schoolCutoff(s,criteria),department=criteria.department&&criteria.department!=='전체'?` · ${criteria.department}`:'';return `<tr><td>${esc(s.name)}${esc(department)} ${editButton(s.name)}</td><td>${esc(result.semesterLabel)}</td><td>${esc(s.region)}</td><td>${esc(s.type)}</td><td>${result.total.toFixed(1)}</td><td>${Number.isFinite(cutoff)&&cutoff>0?cutoff:'-'}</td><td><span class="badge ${style}">${label}</span></td></tr>`; }).join(''):'<tr><td colspan="7">입력한 성적으로 계산할 수 있는 학교가 없습니다. 3-1 또는 전체학기 입시요강과 필수 과목을 확인하세요.</td></tr>'; bindEditButtons();
}
...
function recordPrediction() {
  ...
  const info=currentStudentInfo(),candidates=schoolCandidates(school).filter(x=>x.result.ok);candidates.forEach(({result,criteria})=>{const[label]=judgementForResult(school,result,criteria);predictions.push({student:`${info.grade}학년 ${info.classNo}반 ${info.studentNo}번 ${info.name}`,studentName:info.name,grade:info.grade,classNo:info.classNo,studentNo:info.studentNo,score:result.total.toFixed(1),school:school.name,department:criteria.department||'전체',criteriaId:criteria.id,criteriaVersion:criteria.version,semesterLabel:result.semesterLabel,result:label,at:new Date().toLocaleString('ko-KR')})});saveStats();
}
function renderStats() {
  ...
  $('predictionRows').innerHTML = predictions.length ? predictions.map((x,i) => `<tr><td>${esc(x.student)}</td><td>${x.score}</td><td>${esc(x.school)}</td><td>${esc(x.semesterLabel||'기존 기준')}</td><td>${esc(x.result)}</td><td><button class="secondary delete-prediction" data-index="${i}">삭제</button></td></tr>`).join('') : '<tr><td colspan="6">아직 기록이 없습니다.</td></tr>';
  $('seniorRows').innerHTML = seniorAdmissions.length ? seniorAdmissions.map((x,i) => `<tr><td>${esc(x.graduationYear || '-') }년 · ${esc(x.grade || '-') }학년 ${esc(x.classNo || '-') }반 ${esc(x.no || '-') }번</td><td>${esc(x.school)}</td><td>${x.score}</td><td>${esc(x.result || '합격')}</td><td><button class="secondary delete-senior" data-index="${i}">삭제</button></td></tr>`).join('') : '<tr><td colspan="5">입력된 합격자 자료가 없습니다.</td></tr>';
...
function addSeniorAdmission() { const selected=$('seniorSchool').value, school=selected==='__direct__' ? $('seniorCustomSchool').value.trim() : selected, graduationYear=$('seniorGraduationYear').value, score=Number($('seniorScore').value); if (!graduationYear || !school || !Number.isFinite(score) || score < 0 || score > 100) return alert('졸업년도, 지원학교, 0~100점의 합격점수를 입력하세요.'); seniorAdmissions.push({graduationYear,grade:$('seniorGrade').value,classNo:$('seniorClass').value,no:$('seniorNo').value.trim(),school,score:score.toFixed(1),result:$('seniorResult').value}); $('seniorScore').value = '';$('seniorNo').value='';$('seniorCustomSchool').value=''; saveStats(); renderStats(); renderResults(); renderSearch(); }
function downloadAdmissionTemplate() { const rows=[['졸업년도','학년','반','번호','지원학교','합격점수','결과'],['2025','3','1','1','경주디자인고등학교','85.5','합격']]; if(window.XLSX){const book=XLSX.utils.book_new();XLSX.utils.book_append_sheet(book,XLSX.utils.aoa_to_sheet(rows),'합격자자료');XLSX.writeFile(book,'합격자_자료_양식.xlsx');}else{const a=document.createElement('a');a.href=URL.createObjectURL(new Blob(['\uFEFF'+rows.map(r=>r.join(',')).join('\n')],{type:'text/csv'}));a.download='합격자_자료_양식.csv';a.click();} }
async function importAdmissionFile() { const file=$('excelFile').files[0]; if(!file)return alert('가져올 엑셀 파일을 먼저 선택하세요.'); try { let rows=[]; if(window.XLSX){const data=await file.arrayBuffer(),book=XLSX.read(data,{type:'array'}),sheet=book.Sheets[book.SheetNames[0]];rows=XLSX.utils.sheet_to_json(sheet,{defval:''});}else{const text=await file.text();const lines=text.replace(/^\uFEFF/,'').trim().split(/\r?\n/);const header=lines.shift().split(',');rows=lines.map(line=>Object.fromEntries(header.map((h,i)=>[h,line.split(',')[i]||''])));} let added=0; rows.forEach(r=>{const school=String(r['지원학교']||r['학교명']||r.school||'').trim(),score=Number(r['합격점수']||r.score);if(school&&Number.isFinite(score)&&score>=0&&score<=100){seniorAdmissions.push({graduationYear:String(r['졸업년도']||r.graduationYear||''),grade:String(r['학년']||r.grade||''),classNo:String(r['반']||r.classNo||''),no:String(r['번호']||r.no||''),school,score:score.toFixed(1),result:String(r['결과']||r['합격여부']||r.result||'합격')});added++;}});saveStats();renderStats();renderResults();renderSearch();alert(`${added}건의 합격자 자료를 가져왔습니다.`);}catch(error){alert('파일을 읽지 못했습니다. 양식 파일의 열 이름과 내용을 확인하세요.');}$('excelFile').value=''; }
